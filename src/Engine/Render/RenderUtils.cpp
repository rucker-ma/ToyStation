...
namespace TSEngine {
    RenderUtils::RenderUtils(VkCommandPool Pool)
        :command_pool_(Pool)
    {
    }
    void RenderUtils::CopyBufferToImage(VkBuffer Buffer, VkImage Image, uint32_t Width, uint32_t Height)
    {
    ...
    void RenderUtils::CopyBufferToGPU(void* Data, VkDeviceSize DataSize, VkBuffer& Buffer, VkDeviceMemory& Memory, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties)
    {
        ...
        CreateGPUBuffer(DataSize, usage,
    ...
    void RenderUtils::CreateGPUBuffer(VkDeviceSize Size, VkBufferUsageFlags Usage, VkMemoryPropertyFlags Properties, VkBuffer& Buffer, VkDeviceMemory& BufferMemory)
    {
        ...
        if (vkCreateBuffer(VulkanDevice, &buffer_info, nullptr, &Buffer) != VK_SUCCESS)
        {
        }
        ...
        if (vkAllocateMemory(VulkanDevice, &alloc_info, nullptr, &BufferMemory) != VK_SUCCESS)
        {
        }
        vkBindBufferMemory(VulkanDevice, Buffer, BufferMemory, 0);
    }

    void RenderUtils::TransitionImageLayout(VkImage Image, VkImageLayout OldLayout, VkImageLayout NewLayout,
        VkAccessFlags OldAccessMask,VkAccessFlags NewAccessMask)
    {
        VkCommandBuffer command_buffer = BeginSingletimeCommand();
        TransitionImageLayoutWithCommandBuffer(command_buffer,Image,OldLayout,NewLayout,
            OldAccessMask,NewAccessMask);
        EndSingletimeCommand(command_buffer);
    }

    void RenderUtils::TransitionImageLayoutWithCommandBuffer(VkCommandBuffer CommandBuffer, VkImage Image, VkImageLayout OldLayout,
        VkImageLayout NewLayout, VkAccessFlags OldAccessMask, VkAccessFlags NewAccessMask)
    {
        ...
        vkCmdPipelineBarrier(CommandBuffer, source_stage, dest_stage,
