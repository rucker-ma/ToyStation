...
#define VK_RES_CHECK(res, ...) \
    if (res != VK_SUCCESS)     \
        throw std::runtime_error(std::string(__FILENAME__) + " : " + std::to_string(__LINE__) + ":  vulkan return value error: " + std::to_string(res));
...
namespace TSEngine
{
    ...
    VulkanQueueFamily VulkanContext::FindQueueFamilies(VkPhysicalDevice phy_device)
    {
        ...
        for (const auto& queue_family : queue_families)
        {
            ...
            vkGetPhysicalDeviceSurfaceSupportKHR(phy_device, i,surface_, &presentSupport);
    ...
    void VulkanContext::CreatePresentImage()
    {
        ...
        //present_image_ = ImageFactory::CreateImage(info_.Extend.width, info_.Extend.height, VK_FORMAT_B8G8R8A8_UNORM,
        ...
        //present_image_info_.image = reinterpret_cast<std::uintptr_t>( present_image_->GetVkImage());
        //present_image_info_.image_view = 0;
        //present_image_info_.memory = reinterpret_cast<std::uintptr_t>( present_image_->GetMemory());
    ...
    const VkInstance& VulkanContext::VKInstance()
    {
        return instance_;
    }
    ...
    const VkDevice& VulkanContext::Device()
    {
        return device_;
    }

    //todo: get or set graphics queue when get device
    const VkQueue& VulkanContext::GraphicsQueue()
    {
        return graphics_queue_;
    }

    const VkQueue& VulkanContext::PresentQueue()
    {
        return present_queue_;
    }

    const VkSurfaceKHR& VulkanContext::Surface()
    {
        return surface_;
    }
    ...
    VulkanContext::VulkanContext()
        ...
        debug_utils_messenger_ext_(nullptr),surface_(nullptr)
    {
    }
    ...
    std::vector<const char*> VulkanContext::GetRequiredExtensions()
    {
        ...
        vkEnumerateInstanceExtensionProperties(nullptr, &extensionCount, instance_extensions_.data());
        for (auto& extension : instance_extensions_)
        {
            if (std::string(extension.extensionName) == std::string( "VK_KHR_surface"))
            {
                enable_extensions.push_back(extension.extensionName);
            }
#ifdef _WIN32
            if (std::string(extension.extensionName)== std::string("VK_KHR_win32_surface"))
            {
                enable_extensions.push_back(extension.extensionName);
            }
#endif // _WIN32
    ...
    void VulkanContext::SetupDebugMessenger()
    {
        ...
        auto debug_utils_func = (PFN_vkCreateDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
            instance_, "vkCreateDebugUtilsMessengerEXT");
        if (!debug_utils_func)
        {
            VK_RES_CHECK(VK_ERROR_EXTENSION_NOT_PRESENT)
        }
        VK_PROCESS(debug_utils_func(instance_,&create_info_ext,nullptr,&debug_utils_messenger_ext_))
    }
    ...
    void VulkanContext::CreateSurface(HWND hwnd)
    {
        ...
        VK_PROCESS(vkCreateWin32SurfaceKHR(instance_, &create_info, nullptr, &surface_));
    }
    ...
    void VulkanContext::CreateLogicalDevice()
    {
        ...
        VkResult res = vkCreateDevice(physical_device_, &create_info, nullptr, &device_);
        VK_RES_CHECK(res);
    ...
    bool VulkanContext::SuitableDevice(VkPhysicalDevice Device)
    {
        ...
        {
            VulkanSwapChainSupportDetails swap_support = QuerySwapChainSupport(Device);
            swap_chain_adequate = !swap_support.formats.empty() && !swap_support.present_modes.empty();
        }
    ...
    bool VulkanContext::CheckDeviceExtensionSupport(VkPhysicalDevice Device)
    {
        ...
        vkEnumerateDeviceExtensionProperties(Device, nullptr, &extension_count, avaible_extensions.data());
    ...
    void VulkanContext::SetupDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& create_info_ext)
    {
        create_info_ext = {};
        create_info_ext.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
        create_info_ext.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT |
            VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT |
            VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
        create_info_ext.messageType = VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT |
