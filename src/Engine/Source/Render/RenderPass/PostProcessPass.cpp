...
Matrix4 AxisMatrix(float left,float right,float top,float bottom,float zfar,float znear){
    Matrix4 result(1.0);
...
void PostProcessPass::PostInitialize(){
    axis_model_= std::make_shared<MoveGizmoModel>();
    axis_model_->GenerateModel();
//    MoveGizmoModel model;
    ...
    for(auto& mesh:axis_model_->GetMeshes()) {
        ...
        render_mesh->material_index = mesh->MaterialIndex();
    }

    local_ubo_buffer_ =  context_->GetAllocator()->CreateBuffer(sizeof(UniformBuffer),
                                                               VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
    std::vector<VkWriteDescriptorSet> sets;
    VkDescriptorBufferInfo buffer_info;
    buffer_info.buffer = local_ubo_buffer_.buffer;
    buffer_info.offset = 0;
    buffer_info.range = sizeof(toystation::UniformBuffer);

    auto scissor = context_->GetSwapchain()->GetScissor();

    for(auto& material:axis_model_->GetComponent<MaterialComponent>()->Materials()){
        UniformMaterail uniform_material;

        uniform_material.set_container.Init(
            context_->GetContext().get());
        // set =0,binding=0
        uniform_material.set_container.AddBinding(
            0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
            VK_SHADER_STAGE_VERTEX_BIT|VK_SHADER_STAGE_FRAGMENT_BIT, 0);
        //设置颜色信息，不使用纹理，直接使用Vector颜色
        uniform_material.set_container.AddBinding(1,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1,
                                  VK_SHADER_STAGE_FRAGMENT_BIT,0);
        uniform_material.set_container.AddBinding(2,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,1,
                                  VK_SHADER_STAGE_FRAGMENT_BIT,0);
        uniform_material.set_container.InitLayout();
        uniform_material.set_container.InitPool(1);

        PostProcessUniform uniform_data;
        uniform_data.size = Vector2(scissor->extent.width,scissor->extent.height);
        uniform_data.color = material->Factor().base_color;

        uniform_material.frag_buffer = context_->GetAllocator()->CreateBuffer(cmd,sizeof(uniform_data),&uniform_data,
            VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);

        VkDescriptorBufferInfo frag_uniform_info;
        frag_uniform_info.buffer = uniform_material.frag_buffer.buffer;
        frag_uniform_info.offset = 0;
        frag_uniform_info.range = sizeof(PostProcessUniform);
        sets.clear();
        sets.push_back(uniform_material.set_container.MakeWrite(
            0, 0, &buffer_info));
        sets.push_back(uniform_material.set_container.MakeWrite(
        ...
        uniform_material.set_container.UpdateSets(sets);
        materials_.push_back(uniform_material);
    }
...
void PostProcessPass::Draw() {
    ...
    for(auto& mesh:move_coord_object_->Meshes()) {
        Vector4 color= axis_model_->GetComponent<MaterialComponent>()->Materials()[mesh->material_index]->Factor().base_color;
//        vkCmdPushConstants(cmd,pipelines_[SubPass_Default].layout,VK_SHADER_STAGE_FRAGMENT_BIT,0,
//                           sizeof (Vector4),(void*)&color);
        //UpdateFragUniform(color);
        vkCmdBindVertexBuffers(cmd,0,1,&mesh->position_buffer.buffer,&offset);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                pipelines_[SubPass_Default].layout, 0, 1,
                                &materials_[mesh->material_index].set_container.GetSet(0) , 0, nullptr);
...
bool PostProcessPass::UpdateUniform(){
    uniform_buffer_ = resource_->ubo_;
    auto hitresult = kEngine.GetWorldManager().ActiveLevel()->GetHitResult();
    if(hitresult.hit) {
        // 选中了object,获取object坐标，绘制
        auto center = hitresult.hit_mesh->BoundingBox().Center();
        Vector4 view_pos = uniform_buffer_.view*Vector4 (center,1.0);
        float scale = view_pos.z/200.0f;
        scale = scale<1?1:scale;
        LogDebug(" axis scale is " + std::to_string(scale));
        uniform_buffer_.model = glm::translate(Matrix4(1.0f), center);

        view_pos = uniform_buffer_.proj* view_pos;
        view_pos = view_pos/view_pos.w;
        uniform_buffer_.camera_position = Vector3 (view_pos);
       uniform_buffer_.camera_position.z = scale;
       void*data = context_->GetAllocator()->Map(local_ubo_buffer_);
       memcpy(data,&uniform_buffer_,sizeof(uniform_buffer_));
       context_->GetAllocator()->UnMap(local_ubo_buffer_);
       return true;
    }
    return false;
}
