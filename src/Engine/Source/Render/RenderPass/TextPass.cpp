...
void TextPass::UpdateText(std::string text,Vector2 loc){
    if(vertex_buffer_.buffer){
        context_->GetAllocator()->Destroy(vertex_buffer_);
    }
    current_render_char_.clear();
    PrepareRenderString(loc,std::move(text));
}
TextPass::~TextPass(){
    FT_Done_Face(face_);
