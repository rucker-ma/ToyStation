...
//                GetMaxEncodeHeight() + chromaHeight, 16));
            size_t height = GetMaxEncodeHeight() + chromaHeight;
            size_t pitch = GetWidthInBytes(GetPixelFormat(), GetMaxEncodeWidth());
            m_cudaPitch = pitch;
            CUDA_DRVAPI_CALL(cuMemAlloc((CUdeviceptr *)&pDeviceFrame,height*pitch));
