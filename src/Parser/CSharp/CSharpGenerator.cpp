...
CSharpGenerator::CSharpGenerator(CStyleGenerator* c)
    :cstyle(c)
{
...
void CSharpGenerator::Generate()
{
    ...
    std::string file_path = csharp_folder.append(info.FileName + ".gen.cs").string();
    ...
    for (auto& c : info.Class)
    {
        ...
        else
        {
            ...
            //constructor
            bounds = bounds + "private System.IntPtr _obj;\n   \
                public " + c.ClassName + "(System.IntPtr Ptr)\n{\n  \
             _obj = Ptr;\n}\n";
        ...
        for (auto& func : c.Funcs)
        {
            ...
            content += "[DllImport(\"TSEngine\", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]\n";
            content = content + "internal static extern " + func.ReturnType.TypeName + " " + func.FuncName;
            std::string input = "(";
            int i = 1;
            ...
            if (c.ClassType == DeclType::STATIC)
            {
                ...
                bounds = bounds + "public static " + func.ReturnType.TypeName + " " + func.RawName;
                int i = 1;
                for (auto& param : func.InputParams)
                {
            ...
            else
            {
                int last = 2;
                input_param += "_obj, ";
                bounds = bounds + "public " + func.ReturnType.TypeName + " " + func.RawName;
                for (size_t i = 1; i < func.InputParams.size(); i++)
                {
                    input_param = input_param + func.InputParams[i].ParamName;
                    input = input + func.InputParams[i].TypeName + " " + func.InputParams[i].ParamName;
            ...
            if (func.ReturnType.TypeName == "void") {
                bounds = bounds + input + "\n{\n"  + c.ClassName + "Gen." + func.FuncName + input_param + "\n}\n";
            }
            else {
                bounds = bounds + input + "\n{\n" + " return " + c.ClassName + "Gen." + func.FuncName + input_param + "\n}\n";
            }
