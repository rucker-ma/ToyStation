...
CX_CXXAccessSpecifier MethodCursor::GetAccess()
{
    return access_;
}

std::vector<std::string>& MethodCursor::Keys()
{
    return keys;
}

std::string MethodCursor::ReturnType()
{
    return return_type;
}

std::vector<ParamDecl> MethodCursor::Params()
{
    return params;
}
...
void MethodCursor::Process()
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        //CXType argtype = clang_getArgType(clang_getCursorType(GetCXCursor()), i);
        //std::cerr <<ToString(clang_getTypeKindSpelling(argtype.kind))<<std::endl;
        std::string par_type = ToString(clang_getTypeSpelling(type));
        //std::cerr << par_type << " Kind: " << ToString(clang_getTypeKindSpelling(type.kind)) << std::endl;
        ParamDecl decl;
...
std::vector<Cursor> MethodCursor::ParamCursors()
{
    return param_cursors;
}
...
CX_CXXAccessSpecifier FieldCursor::GetAccess()
{
    return access_;
}

std::string FieldCursor::DeclType()
{
    return decl_type;
}

std::vector<std::string>& FieldCursor::Keys()
{
    return keys;
}
...
void FieldCursor::Process()
{
    ...
    //std::cerr << decl_type << " Kind: " << ToString(clang_getTypeKindSpelling(type.kind)) << std::endl;
}
