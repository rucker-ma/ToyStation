...
CStyleGenerator::CStyleGenerator(FileParser* parser) : IGenerator(parser) {
...
void CStyleGenerator::Generate() {
    ...
    auto RelativePath = std::filesystem::relative(parser_->Path(),
    ...
    if (!impl_file_content.empty()) {
        ...
        std::string raw_file_name =
            parser_->Path().filename().replace_extension().string();
        auto RelativeFolder = std::filesystem::relative(
            parser_->Path().parent_path(), PathEnv::Get().ProjectFolder);
        ...
        std::ofstream hfilestream(hfile.string(),
                                  std::ios::trunc | std::ios::in);
        header_file_content = CodeFormatter::FormatCPP(header_file_content);

        hfilestream.write(header_file_content.c_str(),
                          header_file_content.size());
        hfilestream.close();

        std::filesystem::path cppfile(GenFolder);
        cppfile.append(raw_file_name + "_c.cpp");
        impl_file_content =
            "#include \"" + raw_file_name + "_c.h" + "\"\n" + impl_file_content;

        std::ofstream cppfilestream(cppfile.string(),
                                    std::ios::trunc | std::ios::in);
        impl_file_content = CodeFormatter::FormatCPP(impl_file_content);
        cppfilestream.write(impl_file_content.c_str(),
                            impl_file_content.size());
        cppfilestream.close();
    }
}

FileMetaInfo& CStyleGenerator::GetMetaInfo() { return info; }
