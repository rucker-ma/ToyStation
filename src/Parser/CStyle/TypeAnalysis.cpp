...
std::vector<std::string> TypeAnalysis::GetCompounds()
{
    return compounds;
}
...
void TypeAnalysis::CheckType(CXType& type, Cursor& c, ParamMetaInfo& info)
{
    std::cerr << "type:" << ToString(clang_getTypeSpelling(type))
        << " kind:  " << ToString(clang_getTypeKindSpelling(type.kind)) << std::endl;
    std::cerr << " cursor : " << c.GetSpelling()
        << " kind:  " << c.GetKindSpelling() << std::endl;
    if (type.kind == CXType_Typedef)
    {
        Cursor cur = clang_getTypeDeclaration(type);
        CXType under_type = clang_getTypedefDeclUnderlyingType(cur.GetCXCursor());
...
void TypeAnalysis::CheckRecord(CXType& type, Cursor& cursor)
{
    ...
    clang_Type_visitFields(type, [](CXCursor C, CXClientData client_data) ->CXVisitorResult {
    ...
    CompoundType.insert(std::pair<std::string, StructMetaInfo>(cursor.GetSpelling(), Record));
}
