    ...
    for (int i = 1; i < argc; i++)
    {
        if (std::string(argv[i]) == "-I")
        {
            while (i + 1 < argc)
            {
                ...
                PathEnv::Get().IncludeFolder.push_back("-I"+ std::string(argv[i]));
                std::cerr << "Include Folder :" << std::string(argv[i]) << std::endl;
            }
            res = res ^ 0x01;
        }
        else if (std::string(argv[i]) == "-P")
        {
            ...
            std::cerr << "Project Folder :" << std::string(argv[i]) << std::endl;
            res = res ^ 0x02;
        }
