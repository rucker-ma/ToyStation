...
using MatchFunc = bool (MacroCursor::*)(std::string linestr,
                                        unsigned int &offset);

MacroCursor::MacroCursor() : Cursor() {}

MacroCursor::MacroCursor(const Cursor &cursor) : Cursor(cursor) {}
...
Cursor MacroCursor::GetNextCursor() {
    ...
    ParseKeys();
    CXSourceLocation newloc = clang_getLocationForOffset(
        clang_Cursor_getTranslationUnit(cursor_), file, offset);

    modified_cursor_ =
        clang_getCursor(clang_Cursor_getTranslationUnit(cursor_), newloc);
...
std::vector<std::string> MacroCursor::GetKeys() { return keys; }
...
void MacroCursor::ParseKeys() {
    ...
    CXFile file;
    clang_getSpellingLocation(start_loc, &file, &start_line, &start_column,
                              &start_offset);
    unsigned int end_line, end_column, end_offset;
    clang_getSpellingLocation(end_loc, NULL, &end_line, &end_column,
                              &end_offset);
    size_t content_size;

    const char *data = clang_getFileContents(
        clang_Cursor_getTranslationUnit(GetCXCursor()), file, &content_size);
...
MacrosDelegate::MacrosDelegate() : cursors_{} {}
